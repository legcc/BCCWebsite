    ...
    <div>
        <div class="main-title">
            ...
            <div class="course-skip">
                <nuxt-link :to="{name:'learner-statistics',query:{levelId:+levelId,lessonId:+lessonId}}">
        ...
        <div class="main-content">
            <ul class="exercise-list">
                <li class="exercise-info" v-for="(info,index) in dataOptions.exercises">
                    <nuxt-link v-if="info.exerciseLock==0" :to="{name:EXERCISE_ENTRY, params: exerciseTemplate[index]&&{exercise:exerciseTemplate[index].params, moduleName:exerciseTemplate[index].name}}">
                        <div class="info-start">
...
<script>
    ...
    export default {
        async fetch({query, isServer, store, redirect}){
            ...
            let lessonId = +query.lessonId || +query.lessonId;
            if (isServer && !lessonId) {
                redirect(home);
                return void(0)
            }
            lessonId ? store.dispatch("index/learner/locus/setLessonPath", lessonId) : lessonId = store.state.index.learner.locus.lessonId;
            store.dispatch("index/learner/learner/changeEntryActive", "myCourse");
            await Promise.all([store.dispatch('index/learner/courseDetail/getData', {lessonId: lessonId}),
                store.dispatch('index/learner/courseDetail/getExerciseTemplate', {lessonId: lessonId}),
                store.dispatch('index/learner/courseDetail/getCourseTemplate', {lessonId: lessonId})])
        },
        asyncData({query}){
            return {
...
//            初始化练习音频样式
        ...
        components: {},
        computed: {
            ...mapState('index/learner/courseDetail', {
