...
export const actions = {
    async getData({commit, rootState}, paramData){
        try {
            let res = await Promise.all([
                axios.get(utils.changeParamsToUrl(HTTP_PATH + "/api/progress/getManyClassCompare", 
		        	['uid', 'classIds', 'levelId', 'lessonId'], 
    ...
    async searchData({state, commit, rootState}, paramData){
        try {
            commit("GET_DATA", (await utils.sendAxiosGet("/api/progress/getManyClassCompare",
                    ['uid', 'classIds', 'levelId', 'lessonId'], 
