...
export const actions = {
    async getData({state, commit, rootState}, paramData){
        try {
            let res = await Promise.all([
                axios.get(utils.changeParamsToUrl(HTTP_PATH + "/api/progress/getExerciseProgress",
                    ['uid', 'levelId', 'lessonId', 'page'], {uid: rootState.authUser.uid, ...paramData})),
    ...
    async searchTable({state, commit, rootState}, paramData){
        try {
            commit("GET_DATA", (await axios.get(utils.changeParamsToUrl(HTTP_PATH + `/api/progress/getExerciseProgress`,
                    ['uid', 'levelId', 'lessonId', 'page'],
