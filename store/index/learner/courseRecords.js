...
export const actions = {
    async getData({state, commit, rootState}, proId){
        try {
            let res = await Promise.all([
                axios.get(utils.changeParamsToUrl(HTTP_PATH + "/api/progress/getCourseRecord", ['uid', 'proId', 'levelId', 'lessonId', 'page'], {uid: rootState.authUser.uid, ...proId})),
    ...
    async searchTable({state, commit, rootState}, proId){
        try {
            commit("GET_DATA", (await axios.get(utils.changeParamsToUrl(HTTP_PATH + "/api/progress/getCourseRecord", ['uid', 'proId', 'levelId', 'lessonId', 'page'], {uid: rootState.authUser.uid, ...proId}))).data.data || []);
        } catch (e) {
            console.error(e);
        }
    }
