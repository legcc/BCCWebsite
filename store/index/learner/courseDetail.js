...
export const actions = {
    async getData({state, commit, rootState}, lessonId){
        try {
            state.dataOptions.length || commit("GET_DATA",
                (await axios.get(utils.changeParamsToUrl(HTTP_PATH + "/api/study/getMyStudyDetail",
                    ['uid', 'lessonId'], {uid: rootState.authUser.uid, ...lessonId}))).data.data)
        } catch (e) {
            console.error(e);
        }
    },
    async getExerciseTemplate({state, commit, rootState}, param){
        commit("SET_EXERCISE_TEMPLATE", (await utils.sendAxiosGet("/api/study/getExerciseTemplate", ['uid', 'lessonId'], {...rootState.authUser, ...param})).data.data);
    },
