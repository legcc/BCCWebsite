...
const mockPath = HTTP_PATH.replace('proxy', 'virtualInterface');
...
void function () {
    ...
    utils.extend(true, utils, {
        ...
        isNotEmpty: function (obj) {
            var flag = true;
            if (obj && objTypeRe.exec(toString.call(obj))[1] === "Array") {
                for (let i = 0, length = obj.length; i < length; i++) {
                    if (!obj[i]) flag = false;
                }
            }
            if (obj && objTypeRe.exec(toString.call(obj))[1] === "Object") {
                for (let key in obj) {
        ...
        //转换get请求
        changeParamsToUrl: function (url, keys, params) {
            for (let i = 0, length = keys.length; i < length; i++) {
                let path = params[keys[i]] || 0;
                url += `/${path}`;
            }
            return url;
        },
        sendAxiosPost: function (url, data,
                                 headers = {
        ...
        }, sendNodePost: function (url, data,
                                   headers = {
                                       'Content-Type': 'application/x-www-form-urlencoded'
                                   }) {
            return axios({
        ...
        sendAxiosGet: function (url, keys, params) {
            return axios.get(this.changeParamsToUrl(`${HTTP_PATH}${url}`, keys, params))
        },
        sendMockGet: function (url, keys, params) {
            return axios.get(this.changeParamsToUrl(`${mockPath}${url}`, keys, params))
        },
        /**
         ** 加法函数，用来得到精确的加法结果
